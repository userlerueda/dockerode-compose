...
async function up(docker, projectName, recipe, output, options) {
  ...
  var serviceNames = tools.sortServices(recipe);
  for (var serviceName of serviceNames) {
    var configHash = servicesTools.buildSHA256(JSON.stringify(recipe.services[serviceName]))
    var pathScope = {};
    ...
    var containerInfo
    let currentContainers = await docker.listContainers({all: true, filters: `{"name":["${opts.name}"]}`})
    if (currentContainers.length) {
      if (currentContainers[0].Labels["com.docker.compose.config-hash"] == configHash && currentContainers[0].State == "running") {
        containerInfo = currentContainers[0]
      } else {
        console.debug(`Recreating container ${opts.name}...`)
