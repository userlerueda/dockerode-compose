...
export function getImages(recipe: DockerComposeRecipe) {
  ...
  if (recipe.services !== undefined) {
    ...
    for (var serviceName of serviceNames) {
      let imageName = recipe.services[serviceName].image;
...
export function sortServices(recipe: DockerComposeRecipe): any[] {
  let order: any[] = [];
...
export function sortNetworksToAttach(networksToAttach: ComposeOutput['networks']) {
  var networksToAttachSorted: any[] = [];
  if (networksToAttach !== undefined) {
      ...
      if (i === 0) {
        ...
        for (let j = 0; j < networksToAttachSorted.length; j++) {
          ...
          if (networksToAttachSorted[j][networkNameSorted].priority > networksToAttach[i][networkName].priority) {
            ...
            networksToAttachSorted[j][networkNameSorted].priority < networksToAttach[i][networkName].priority
          ) {
            aux += j - 1;
          } else {
            aux += j + 1;
