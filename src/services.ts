...
import { ComposeDownOptions, ComposeOutput, ComposeUpOptions, DockerComposeRecipe } from './models/dockerCompose';
...
export module Services {
  ...
  export async function up(
    ...
    let services: Dockerode.Container[] = [];
    if (recipe.services !== undefined) {
      ...
      for (var serviceName of serviceNames) {
        ...
        if (isServiceUpToDate && existingContainer !== undefined) {
        ...
        if (service.networks !== undefined) {
          ...
          serviceNetworks.map((serviceNetwork) => {
            if (
              output.networks !== undefined &&
              output.networks.filter((network) => network.name === serviceNetwork).length === 0
            ) {
          ...
          if (output.networks !== undefined) {
            console.info('Service has no networks to attach to, attaching to default network...');
            let defaultProjectNetwork = output.networks.filter(
              (network) => network.isDefault !== undefined && network.isDefault === true,
            );
            console.debug(`Found default network: ${JSON.stringify(defaultProjectNetwork)}`);
            if (defaultProjectNetwork.length == 0) {
              ...
              _.set(opts, 'HostConfig.NetworkMode', defaultProjectNetwork[0].name);
              _.set(opts, 'NetworkingConfig.EndpointsConfig' + defaultProjectNetwork[0].name, {
                IPAMConfig: null,
                Links: null,
                Aliases: [serviceName],
              });
            ...
            console.warn("No networks defined in the compose file, can't attach to any network!");
          }
        }
        ...
        if (networksToAttach.length > 1) {
          ...
          let attachedNetwork = findNetwork(output, networkNames[0]);
          ...
          for (const networkToAttach of networksToAttachSorted) {
            ...
            let network = await findNetwork(output, networkName);
            if (network !== null) {
              network.connect({
                Container: container.id,
